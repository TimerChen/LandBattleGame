...
public:
	...
	double MissileGetScore( const MoveData &mdata )
	{
		...
		if( abs(mdata.x[3]) == 1 || abs(mdata.x[3]) == 2 )
		{
			if( Count[ mdata.x[2] ] == 3 )
				return -GScore( mdata.x[3] );
			else if( Count[ mdata.x[2] ] == 2 )
				return -0.3*GScore( mdata.x[2] )-GScore( mdata.x[3] ); 
			else if( Count[ mdata.x[2] ] == 1 )
				return -0.5*GScore( mdata.x[2] )-GScore( mdata.x[3] ); 
		}else if( abs(mdata.x[3]) <= 8 )
		{
			if( abs(mdata.x[3]) == Count[ -type*1 ] + Count[ -type*2 ] == 0 )
			{
				int i;
				for( i = 3; i <= 8; i++ )
					if( Count[ -type*i ] )break;
				if( i == abs( mdata.x[3] ) )
					return -Score[ mdata.x[3] ]*0.5;//?
				//else return Direct
			}
			...
			if( Count[ mdata.x[2] ] == 3 )
				...
				return -0.9*GScore( mdata.x[2] )-GScore( mdata.x[3] ); 
			else if( Count[ mdata.x[2] ] == 1 )
...
int main(int argc, char** argv) {
    ...
    while (true) {
        ...
        if (op == "id") {
			...
			cerr << id << endl;
			if( rand()%2 )
            	cout << "Excalibur ver3.1.1 [??????????]" << endl;
            else 
            	cout << "[??????????] 1.1.3rev rubilacxE" << endl;
            end();
		} else if (op == "refresh") {
			get_init();
			initialize();
